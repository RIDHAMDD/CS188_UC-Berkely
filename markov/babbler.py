...
# Ridham Dholaria 
# I have used copilot to write comments for me.
...
class Babbler:
    ...
    def __init__(self, n, seed=None):
        ...
        # need to store values (listes or bag; both allow duplicates, one is ordered, the other is not)
            # graph = {
    ...
    def add_sentence(self, sentence):
        ...
        """
        words = sentence.lower().split() #splits the sentence into words and makes them lowercase
        length = len(words) #gets the length of the words
        
        if length >= self.n: #checks if the length of the words is greater than or equal to the size of the n-grams
            starter = " ".join(words[:self.n]) #gets the first n words of the sentence
            self.starters.append(starter) #adds the starter to the list of starters
        
        if length >= self.n: #checks if the length of the words is greater than or equal to the size of the n-grams
            stopper = " ".join(words[-self.n:]) #gets the last n words of the sentence
            self.stoppers.append(stopper) #adds the stopper to the list of stoppers
            
            self.brainGraph.setdefault(stopper, []).append("EOL") #adds the stopper to the brainGraph and sets the value to EOL
        
        till = length - self.n #gets the length of the words minus the size of the n-grams
        for i in range(till): #goes to the length of the words minus the size of the n-grams as ngram is i to i + self.n, this makes sures that thre is no indexoutofbounds error
            ngram = " ".join(words[i:i + self.n]) #gets the n-gram
            successor = words[i + self.n] #gets the successor of the n-gram

            if ngram not in self.brainGraph: #checks if the ngram is not in the brainGraph
                self.brainGraph[ngram] = [successor] #adds the ngram to the brainGraph and sets the value to the successor
            else:
                self.brainGraph[ngram].append(successor) #adds the successor to the ngram in the brainGraph
    ...
    def get_starters(self):
        ...
        """
        return list(set(self.starters)) #returns the starters of the brainGraph
    ...
    def get_stoppers(self):
        ...
        multiple sentences. Probably a one-line method.
        """
        return list(set(self.stoppers)) #returns the stoppers of the brainGraph
    ...
    def get_successors(self, ngram):
        ...
        """
        return list(set(self.brainGraph.get(ngram, []))) #returns the successors of the ngram
    ...
    def get_all_ngrams(self):
        ...
        """
        return list(self.brainGraph.keys()) #returns all the keys in the brainGraph
    ...
    def has_successor(self, ngram):
        ...
        Probably a one-line method.
        """
        return ngram in self.brainGraph #checks if the ngram is in the brainGraph
    ...
    def get_random_successor(self, ngram):
        ...
        """
        successors = self.brainGraph.get(ngram, []) #gets the successors of the ngram
        return random.choice(successors) #returns a random successor
    
    def get_random_ngram(self):
        """
        Returns a random starter ngram.
        """
        return random.choice(self.starters)   #returns a random starter
    ...
    def babble(self):
        ...
        """
        currentNgram = self.get_random_ngram() #gets a random starter
        currentSentance = currentNgram.split() #splits the current ngram into a list

        while True:
            succesor = self.get_random_successor(currentNgram) #gets a random successor
            
            if succesor == 'EOL':
                return " ".join(currentSentance) #returns the current sentence

            currentSentance.append(succesor) #adds the successor to the current sentence
            currentNgram = " ".join(currentSentance[-self.n:]) #gets the last element of the sentence     
